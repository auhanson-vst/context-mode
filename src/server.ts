...
EXAMPLE: ctx_execute(language: "javascript", code: "const out = require('child_process').execSync('gh issue list --json number,title --limit 100', {encoding:'utf8'}); const hooks = JSON.parse(out).filter(i => /hook|routing/i.test(i.title)); console.log(\`\${hooks.length} hook-related issues\`)")`,
    inputSchema: z.object({
      ...
      timeout: z
        .coerce.number()
        .optional()
        .describe("Max execution time in ms. When omitted, a bounded default applies under Antigravity CLI and Claude Code (whose host RPC timeout is near-infinite; see Issue #936) — tunable via CONTEXT_MODE_CLAUDE_EXEC_TIMEOUT_MS / CONTEXT_MODE_AGY_EXEC_TIMEOUT_MS / CONTEXT_MODE_DEFAULT_EXEC_TIMEOUT_MS. On other hosts the MCP host's RPC timeout governs. Pass an explicit value for long-running builds (Gradle/Maven/SBT)."),
...
EXAMPLE: ctx_execute_file(path: "data.csv", language: "javascript", code: "const rows = FILE_CONTENT.split('\\\\n'); console.log(\`rows: \${rows.length - 1}, header: \${rows[0]}\`)")`,
    inputSchema: z.object({
      ...
      timeout: z
        .coerce.number()
        .optional()
        .describe("Max execution time in ms. When omitted, a bounded default applies under Antigravity CLI and Claude Code (Issue #936); on other hosts the MCP host's RPC timeout governs."),
      intent: z
        .string()
        .optional()
        .describe(
  ...
  concurrency: 2
    ...
    inputSchema: z.object({
      ...
      timeout: z
        .coerce.number()
        .optional()
        .describe("Max execution time in ms. When omitted, a bounded default applies under Antigravity CLI and Claude Code (Issue #936); on other hosts the MCP host's RPC timeout governs. With concurrency=1, the value (when set) is a shared budget across commands; with concurrency>1, it is applied per-command."),
      concurrency: z
        .coerce.number()
        .int()
        .min(1)
